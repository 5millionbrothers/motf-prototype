(function loadPublicCatalog() {
  ...
  function regionOf(business) {
    return business.region || business.address?.split(/[ ,]/).filter(Boolean)[0] || "지역 미정";
  }
  ...
  function buildStay(business, offerings, index) {
    const rooms = offerings.map((item) => ({
      ...
      image: item.image_url || business.cover_image_url || roomFallback,
    ...
    return {
      ...
      rating: 0,
      reviews: 0,
      distance: business.address || "주소 문의",
      ...
      intro: business.description || "단체 행사 이용이 가능한 제휴 숙소입니다.",
      amenities: business.facilities?.length ? business.facilities : ["상세 시설은 사장님에게 문의해주세요."],
  ...
  function buildStore(business, offerings, index) {
    const products = offerings.map((item) => ({
      ...
      price: Number(item.price) || 0,
      origin: "업체 제공 상품",
      image: item.image_url || business.cover_image_url || productFallback,
      detail: item.description || "상세 내용은 공판장에 문의해주세요.",
    }));
    return {
      id: business.id,
      name: business.business_name,
      ...
      image: business.cover_image_url || products[0]?.image || marketFallback,
      intro: business.description || "단체 행사 물품을 준비할 수 있는 제휴 공판장입니다.",
      products,
    };
  }
