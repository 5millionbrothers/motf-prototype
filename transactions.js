(function connectUserTransactions() {
  ...
  const isUuid = (value) => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{12}$/i.test(value || "");
  const statusText = {
    pending: "결제 완료·승인 대기",
    confirmed: "확정",
    rejected: "거절",
  ...
  function readVirtualAccount(account = {}) {
    return {
      bank: account.bankName || account.bank || account.bankCode || account.bank_code || "",
      accountNumber: account.accountNumber || account.account_number || account.number || "",
      holder: account.holderName || account.accountHolder || account.account_holder || account.customerName || "",
      due: account.dueDate || account.due_date || account.expiredAt || account.expiresAt || account.expiry || "",
    };
  }
  ...
  async function loadMyTransactions() {
    ...
    const [reservationResult, orderResult, intentResult] = await Promise.all([
      client.from("reservations")
        .select("id, event_date, check_in_date, check_out_date, guest_count, offering_name, total_amount, status, refund_status, refund_amount, businesses(business_name)")
    ...
    if (reservationResult.error || orderResult.error || intentResult.error) {
      console.warn("내 거래 내역을 불러오지 못했습니다.", reservationResult.error || orderResult.error || intentResult.error);
      return;
    }
    ...
    const reservations = (reservationResult.data || []).map((item) => ({
      ...
      date: item.check_in_date || item.event_date,
    ...
    (intentResult.data || []).forEach((item) => {
      const account = readVirtualAccount(item.virtual_account || {});
      const pendingItem = {
        id: item.order_id,
        amount: item.amount,
        status: "입금 대기",
  ...
  document.addEventListener("submit", async (event) => {
    if (event.target.id === "bookingForm") {
      ...
      if (submitButton) {
        ...
        submitButton.textContent = "결제 금액 확인 중...";
      }
  ...
  document.addEventListener("click", (event) => {
    ...
    if (form && !form.checkValidity()) {
      window.setTimeout(() => alert("필수 입력 항목과 동의 체크를 모두 확인해 주세요."), 0);
    }
  }, true);
