...
document.addEventListener("click", async (event) => {
  ...
  if (stayPageButton && !stayPageButton.disabled) {
    ...
    renderStays();
    qs("#stayList")?.closest(".split-layout")?.scrollIntoView({ behavior: "smooth", block: "start" });
    return;
  }
...
(async function boot() {
  ...
  try {
    const params = new URLSearchParams(window.location.search);
    const isPaymentRedirect = params.has("paymentKey") || params.has("code");
    if (!isPaymentRedirect) navigate(routeFromLocation(), { record: false, replace: true });
    await loadPaymentConfig();
    if (isPaymentRedirect) await handleTossRedirect();
    await Promise.all([loadFavoriteStaysFromDatabase(true), loadFavoriteEventsFromDatabase(true)]);
    refreshFavoriteViews();
    updateCartBadge();
    refreshIcons();
