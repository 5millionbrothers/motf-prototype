(function loadPublicCatalog() {
  ...
  const catalogCacheKey = "motf.publicCatalog.v2";
  try {
    const cached = JSON.parse(localStorage.getItem(catalogCacheKey) || "null");
    if (cached?.savedAt && Date.now() - cached.savedAt < 24 * 60 * 60 * 1000 && Array.isArray(cached.stays) && Array.isArray(cached.stores)) {
      window.motfApplyCatalog(cached.stays, cached.stores);
    }
  } catch { localStorage.removeItem(catalogCacheKey); }
  ...
  (async () => {
    ...
    try { localStorage.setItem(catalogCacheKey, JSON.stringify({ savedAt: Date.now(), stays, stores })); } catch { /* Cache is only a rendering optimization. */ }
  })();
})();
