...
async function confirmPaymentOnServer(payment, params = new URLSearchParams(), portoneResponse = null) {
  ...
  try {
    ...
    const wrapped = new Error(`우리 서버 결제 확인 API 호출 실패: ${error.message || "네트워크 오류"}`);
    wrapped.diagnostics = { stage: "browser_fetch", paymentId: payload.paymentId, orderId: payload.orderId };
    throw wrapped;
  }
  ...
  const data = await response.json().catch(() => ({
    ...
    diagnostics: { stage: "parse_confirm_response", paymentId: payload.paymentId, orderId: payload.orderId },
  }));
  if (response.ok && data.ok) return data;

  const error = new Error(data.message || `Payment confirmation failed. HTTP ${response.status}`);
  error.statusCode = response.status;
  error.diagnostics = data.diagnostics || data.debug || null;
  ...
  try {
    ...
    try {
      ...
      const failureDiagnostics = {
        ...
        statusCode: confirmError.statusCode || null,
        diagnostics: confirmError.diagnostics || null,
        portoneResponseKeys: response && typeof response === "object" ? Object.keys(response).slice(0, 30) : [],
      };
      try {
        window.localStorage.setItem("motf.lastConfirmPaymentError", JSON.stringify(failureDiagnostics));
      } catch {}
      console.error("MOTF_CONFIRM_PAYMENT_DIAGNOSTICS", failureDiagnostics);
      showVirtualAccountIssued(
        responseAccount,
        hasBrowserAccount
          ? "가상계좌는 발급되었습니다. 입금 완료 후 예약 요청 상태로 전환됩니다."
          : `포트원 결제창 호출은 완료되었습니다. 서버 확인 실패: ${confirmError.message || "원인 확인 필요"}${confirmError.diagnostics?.stage ? ` (${confirmError.diagnostics.stage})` : ""}`,
        hasBrowserAccount
      );
    }
  } catch (error) {
